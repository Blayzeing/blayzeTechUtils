...
public class FullyConnectedSigmoidLattice {
	 ...
	 * @return	double[]	every weight in this lattice as one double[] in the form [inputWeights | weights depth 0, 1, 2, 3..., n | outputWeights]
	 */
	public double[] getWeightsOld()
	{
		double[] output = new double[inputCount * layerWidth + layerWidth * layerWidth * networkDepth + outputCount * layerWidth];
		System.arraycopy(inputWeights, 0, output, 0, inputWeights.length);
		for( int i = 0; i<weights.length; i++)
			System.arraycopy(weights[i], 0, output, inputWeights.length + i * weights[0].length, weights[0].length);
		System.arraycopy(outputWeights, 0, output, inputWeights.length + weights.length * weights[0].length, outputWeights.length);
		return output;
	}
	/**
	 * Returns the entire weight set for this lattice.
	 ...
	 * @param	newWeights	An array in the same format as that seen in 'getWeights()' <br> NOTE: This must be the correct size.
	 */
	public void setWeightsOld(double[] newWeights)
	{
		System.arraycopy(newWeights, 0, inputWeights, 0, inputWeights.length);
		for(int i = 0; i<weights.length; i++)
			System.arraycopy(newWeights, inputWeights.length + i * weights.length, weights[i], 0, weights[i].length);
		System.arraycopy(newWeights,  inputWeights.length + weights.length * weights[0].length, outputWeights, 0, outputWeights.length);
	}
	/**
